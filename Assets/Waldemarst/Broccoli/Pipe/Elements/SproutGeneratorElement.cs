...
namespace Broccoli.Pipe {
	...
	public class SproutGeneratorElement : PipelineElement, ISproutGroupConsumer {
		...
		public SproutGeneratorElement () {
			this.elementName = "Sprout Generator";
			this.elementHelpURL = "https://docs.google.com/document/d/1Nr6Z808i7X2zMFq8PELezPuSJNP5IvRx9C5lJxZ_Z-A/edit#heading=h.m6d7mqd1neq0";
			this.elementDescription = "This node displays the options to spawn sprout positions and orientation parameters along the tree leves.";
		}
		#endregion
		...
		public int[] GetGroupIds () {
			int[] ids = new int [sproutSeeds.Count];
			for (int i = 0; i < sproutSeeds.Count; i++) {
				ids [i] = sproutSeeds [i].groupId;
			}
			return ids;
		}
		/// <summary>
		...
		public Color[] GetGroupColors () {
			Color[] colors = new Color [sproutSeeds.Count];
			for (int i = 0; i < sproutSeeds.Count; i++) {
				if (pipeline != null) {
					colors [i] = pipeline.sproutGroups.GetSproutGroupColor (sproutSeeds [i].groupId);
				} else {
