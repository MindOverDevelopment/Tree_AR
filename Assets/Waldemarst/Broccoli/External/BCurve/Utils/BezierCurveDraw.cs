...
namespace Broccoli.Utils
{
	...
	public class BezierCurveDraw {
        ...
        /// Size on nodes.
        /// </summary>
        public static float nodeSize = 0.06f;
		/// <summary>
        /// Size on points.
        /// </summary>
		...
		public static void DrawCurveNodes (BezierCurve curve, Vector3 offset, float scale, Color color, bool showNodeForward = false, bool showNodeUp = false) {
			#if UNITY_EDITOR
			Vector3 currentNodePos = Vector3.zero;
			float handleSize = 0f;
			_tmpColor = Handles.color;

			for (int i = 0; i < curve.nodes.Count; i++) {
				currentNodePos = curve.nodes[i].position;
				handleSize = HandleUtility.GetHandleSize (currentNodePos);
				_tmpColor = Handles.color;
				...
				if (currentNodePos == null)
					Debug.Log ("Node pos is null");
				Handles.DrawSolidDisc ((currentNodePos * scale) + offset,
                        Camera.current.transform.forward, 
                        handleSize * nodeSize);
				if (showNodeForward) {
					Handles.color = pointForwardColor;
					Handles.ArrowHandleCap (-1, (currentNodePos * scale) + offset, Quaternion.LookRotation (curve.nodes[i].direction), 
						handleSize * vectorSize, EventType.Repaint);
				}
				if (showNodeUp) {
					Handles.color = pointUpColor;
					Handles.ArrowHandleCap (-1, (currentNodePos * scale) + offset, Quaternion.LookRotation (curve.nodes[i].up), 
						handleSize * vectorSize, EventType.Repaint);
				}
			}
			...
			Color color, bool showPointForward = false, bool showPointNormal = false, bool showPointUp = false, bool showPointTangent = false)
		{
			...
			for (int i = 0; i < curve.points.Count; i++) {
				...
				if (showPointUp) {
					Handles.color = pointUpColor;
					Handles.ArrowHandleCap (-1, (currentPoint * scale) + offset, Quaternion.LookRotation (curve.points[i].bitangent), 
						handleSize * vectorSize, EventType.Repaint);
				}
		...
		public static void DrawCurveFromSamples (BezierCurve curve, Vector3 offset, float scale, Color color)
		{
			#if UNITY_EDITOR
			Vector3 currentPoint = Vector3.zero;
			...
			for (int i = 0; i < curve.points.Count - 1; i++) {
				currentPoint = curve.points[i].position;
				nextPoint = curve.points[i + 1].position;

				// Draw line
				Handles.DrawLine ((currentPoint * scale) + offset, (nextPoint * scale) + offset);
			}
			Handles.color = _tmpColor;
			#endif
		}
