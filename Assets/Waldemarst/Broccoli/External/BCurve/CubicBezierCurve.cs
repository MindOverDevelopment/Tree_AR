    ...
    public class CubicBezierCurve {
        ...
        */
        ...
        private void ApplyNoise (CurvePoint curvePoint) {
            //float noiseSmoothness = 1;
            /*
            float pointNoiseX = 
                (curvePoint.lengthPosition * noiseLengthScale + noiseLengthOffset + 0.5f) * 
                Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
            float pointNoiseY = 
                (curvePoint.lengthPosition * noiseLengthScale + noiseLengthOffset) * 
                Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
                */
                /*
            float pointCoord = (curvePoint.lengthPosition * 
                noiseLengthScale * 
                Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition));
            float pointNoiseX = 
                pointCoord + 
                (noiseLengthOffset + 0.5f) * noiseLengthScale;
            float pointNoiseY = 
                pointCoord + 
                noiseLengthOffset * noiseLengthScale;
                */
