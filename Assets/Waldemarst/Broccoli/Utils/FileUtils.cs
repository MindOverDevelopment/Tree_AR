...
namespace Broccoli.Utils
{
    ...
    public class FileUtils {
        ...
        public static string GetFilePath (string folderPath, string filename, string ext, int take = -1) {
            return Path.Combine (folderPath, filename) + (take<0?"":GetFileSuffix (take)) + "." + ext;
        }
        /// <summary>
        ...
        public static string GetFileSuffix (int take) {
            if (take < 0) take = 0;
            string suffix;
            if (take < 10) suffix = $"00{take}";
            else if (take < 100) suffix = $"0{take}";
            else suffix = $"{take}";
            return suffix;
        }
        /// <summary>
