...
namespace Broccoli.TreeNodeEditor
{
    ...
    */
    ...
    public class StructureEdgeDragHelper<TEdge> : EdgeDragHelper where TEdge : Edge, new()
    {
        ...
        const int k_PanAreaWidth = 25;
        const int k_PanSpeed = 4;
        const int k_PanInterval = 10;
        const float k_MinSpeedFactor = 0.5f;
        const float k_MaxSpeedFactor = 2.5f;
        const float k_MaxPanSpeed = k_MaxSpeedFactor * k_PanSpeed;

        public StructureEdgeDragHelper(IEdgeConnectorListener listener)
        {
            m_Listener = listener;
