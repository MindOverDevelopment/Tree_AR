...
{
    public class StructureSelectionDragger : Dragger
    {
        public delegate Rect MoveFilter (GraphElement element, Rect originalPos, Rect newPos);
        public delegate void Moved (GraphElement element, Rect originalPos, Rect newPos);
        public MoveFilter moveFilter;
        public Moved onBeforeMove;
        public Moved onMove;
        ...
        public StructureSelectionDragger()
        {
        ...
        internal const int k_PanAreaWidth = 25;
        ...
        void MoveElement(GraphElement element, Rect originalPos)
        {
            ...
            if (moveFilter != null) {
                newPos = moveFilter (element, originalPos, newPos);
                onBeforeMove?.Invoke (element, originalPos, newPos);
                element.SetPosition(m_GraphView.contentViewContainer.ChangeCoordinatesTo(element.hierarchy.parent, newPos));
                onMove?.Invoke (element, originalPos, newPos);
            } else {
                onBeforeMove?.Invoke (element, originalPos, newPos);
                element.SetPosition(m_GraphView.contentViewContainer.ChangeCoordinatesTo(element.hierarchy.parent, newPos));
                onMove?.Invoke (element, originalPos, newPos);
