...
{
	/// <summary>
	/// Sparsing transform element editor.
	/// </summary>
	[CustomEditor(typeof(SparsingTransformElement))]
	public class SparsingTransformElementEditor : PipelineElementEditor {
		#region Vars
		/// <summary>
		...
		protected override void OnEnableSpecific () {
			sparsingTransformElement = target as SparsingTransformElement;
		...
		protected override void OnInspectorGUISpecific () {
			...
			if (sparsingTransformElement.selectedSparseLevelIndex != sparseLevelsList.index &&
				sparsingTransformElement.selectedSparseLevelIndex < sparseLevelsList.count) {
				sparseLevelsList.index = sparsingTransformElement.selectedSparseLevelIndex;
			}
		...
		private void DrawSparseLevelItemElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (isActive) {
				if (index != sparsingTransformElement.selectedSparseLevelIndex) {
					sparsingTransformElement.selectedSparseLevelIndex = index;
				}
		...
		{
			Undo.RecordObject (sparsingTransformElement, "Sparse Level selected");
			sparsingTransformElement.selectedSparseLevelIndex = list.index;
		}
		/// <summary>
		/// Adds a list item.
		/// </summary>
		...
		private void OnRemoveSparseLevelItem(ReorderableList list)
		{
			...
			Undo.RecordObject (sparsingTransformElement, "Sparse Level removed");
			sparsingTransformElement.sparseLevels.RemoveAt (list.index);
			Undo.RecordObject (sparsingTransformElement, "Sparse Level removed");
			sparsingTransformElement.selectedSparseLevelIndex = -1;
		...
		private void AddDropdownMenu (Rect rect, ReorderableList list) {
			...
			for (int i = 0; i < 6; i++) {
				bool exists = false;
				for (int j = 0; j < sparsingTransformElement.sparseLevels.Count; j++) {
					if (sparsingTransformElement.sparseLevels[j].level == i) {
						exists = true;
					}
				}
				if (!exists) {
		...
		private void clickHandler (object reference) {
			...
			Undo.RecordObject (sparsingTransformElement, "Sparse Level added");
			sparsingTransformElement.sparseLevels.Add (
				sparseLevel);
		}
		#endregion
	}
