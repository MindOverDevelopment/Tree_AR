...
namespace Broccoli.TreeNodeEditor
{
	...
	/// <summary>
	/// Positioner element editor.
	/// </summary>
	[CustomEditor(typeof(PositionerElement))]
	public class PositionerElementEditor : PipelineElementEditor {
		#region Vars
		/// <summary>
		...
		protected override void OnEnableSpecific () {
			positionerElement = target as PositionerElement;
		...
		protected override void OnSceneGUI (SceneView sceneView) {
			if (positionerElement.useCustomPositions) {
				if (positionerElement.selectedPositionIndex >= 0) {
					// Handle
					EditorGUI.BeginChangeCheck ();
					Vector3 newTargetPosition = Handles.PositionHandle (
						positionerElement.positions [positionerElement.selectedPositionIndex].rootPosition +
                       TreeFactoryEditorWindow.editorWindow.treeFactory.transform.position,
                       Quaternion.identity);
					if (EditorGUI.EndChangeCheck ()) {
						Undo.RecordObject (positionerElement, "Tree position moved.");
						positionerElement.positions [positionerElement.selectedPositionIndex].rootPosition = 
				...
				for (int i = 0; i < positionerElement.positions.Count; i++) {
					...
					Vector3 handlePosition = positionerElement.positions[i].rootPosition + 
						TreeFactoryEditorWindow.editorWindow.treeFactory.transform.position;
					...
					Handles.ArrowHandleCap (0, positionerElement.positions[i].rootPosition + 
						TreeFactoryEditorWindow.editorWindow.treeFactory.transform.position
		...
		protected override void OnInspectorGUISpecific () {
			...
			if (propUseCustomPositions.boolValue) {
				if (positionerElement.selectedPositionIndex != positionsList.index &&
					positionerElement.selectedPositionIndex < positionsList.count) {
					positionsList.index = positionerElement.selectedPositionIndex;
				}
		...
		private void DrawListItemElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (isActive) {
				if (index != positionerElement.selectedPositionIndex) {
					positionerElement.selectedPositionIndex = index;
					SceneView.RepaintAll ();
				}
				EditorGUILayout.Space ();
		...
		private void OnAddListItem (ReorderableList list)
		{
			...
			Undo.RecordObject (positionerElement, "Tree position added");
			positionerElement.positions.Add (position);
		}
		/// <summary>
		...
		{
			Undo.RecordObject (positionerElement, "Tree position selected");
			positionerElement.selectedPositionIndex = list.index;
		}
		/// <summary>
		...
		private void OnRemoveListItem (ReorderableList list)
		{
			...
			Undo.RecordObject (positionerElement, "Tree position removed");
			positionerElement.positions.RemoveAt (list.index);
			Undo.RecordObject (positionerElement, "Tree position selected");
			positionerElement.selectedPositionIndex = -1;
