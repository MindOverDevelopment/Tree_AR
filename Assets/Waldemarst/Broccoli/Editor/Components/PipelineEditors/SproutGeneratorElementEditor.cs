...
{
	/// <summary>
	/// Sprout generator element editor.
	/// </summary>
	[CustomEditor(typeof(SproutGeneratorElement))]
	public class SproutGeneratorElementEditor : PipelineElementEditor {
		#region Vars
		/// <summary>
		...
		protected override void OnEnableSpecific () {
			sproutGeneratorElement = target as SproutGeneratorElement;
		...
		protected override void OnSceneGUI (SceneView sceneView) {
			...
			TreeEditorUtils.DrawTreeSproutsForStructureLevel (
				sproutGeneratorElement.GetInstanceID (), 
		...
		private void DrawListItemElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			Color groupColor = sproutGeneratorElement.GetSproutGroupColor (groupId);
			rect.y += 2;
			...
			if (isActive) {
				...
				int sproutGroupIndex = EditorGUILayout.Popup ("Sprout Group",
					sproutGeneratorElement.pipeline.sproutGroups.GetSproutGroupIndex (groupId),
					sproutGeneratorElement.pipeline.sproutGroups.GetPopupOptions ());
				int selectedSproutGroupId = 
					sproutGeneratorElement.pipeline.sproutGroups.GetSproutGroupId (sproutGroupIndex);
		...
		private void AddListItem(ReorderableList list)
		{
			...
			sproutGeneratorElement.AddSproutSeed (sproutSeed);
		...
		{
			sproutGeneratorElement.RemoveSproutSeed (list.index);
			EditorUtility.SetDirty (target);
		}
		#endregion
	}
