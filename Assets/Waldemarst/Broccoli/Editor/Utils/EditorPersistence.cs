...
namespace Broccoli.Utils
{
	...
	public class EditorPersistence<T> where T: ScriptableObject, IClonable<T> {
		...
		public string btnNewElementTpl = "Create New {0}";
		public string btnNewElementHelpTpl = "Creates a new {0} to work with.";
		public string dialogNewElementTitleTpl = "New {0}";
		public string dialogNewElementMsgTpl = "By creating a new {0} you will lose any changes " +
			"not saved on the current one. Do you want to continue creating a new {0}?";
		public string dialogNewElementBtnOkTpl = "Yes, create a new {0}";
		public string dialogElementBtnCancelTpl = "No";
		public string btnLoadElementTpl = "Load {0} from File";
		public string btnLoadElementHelpTpl = "Loads a {0} from a file.";
		public string dialogLoadElementTitleTpl = "Load {0}";
		public string dialogLoadElementErrorTpl = "Failed to load {0}: The file at the specified path " +
			"is not a valid save file or does not contain {0} data.";
		public string btnSaveAsNewElementTpl = "Save as New {0}";
		public string btnSaveAsNewElementHelpTpl = "Saves the current {0} to a new file.";
		public string dialogSaveAsNewElementTitleTpl = "Save as New {0}";
		public string btnSaveCurrentElementTpl = "Save {0}";
		#endregion
		...
		public void InitMessages () {
			// Create new element.
			btnNewElement = string.Format (btnNewElementTpl, elementName);
			btnNewElementHelp = string.Format (btnNewElementHelpTpl, elementName);
			dialogNewElementTitle = string.Format (dialogNewElementTitleTpl, elementName);
			dialogNewElementMsg = string.Format (dialogNewElementMsgTpl, elementName);
			dialogNewElementBtnOk = string.Format (dialogNewElementBtnOkTpl, elementName);
			// Load from file.
			btnLoadElement = string.Format (btnLoadElementTpl, elementName);
			btnLoadElementHelp = string.Format (btnLoadElementHelpTpl, elementName);
			dialogLoadElementTitle = string.Format (dialogLoadElementTitleTpl, elementName);
			dialogLoadElementError = string.Format (dialogLoadElementErrorTpl, elementName);
			// Save as new element.
			btnSaveAsNewElement = string.Format (btnSaveAsNewElementTpl, elementName);
			btnSaveAsNewElementHelp = string.Format (btnSaveAsNewElementHelpTpl, elementName);
			dialogSaveAsNewElementTitle = string.Format (dialogSaveAsNewElementTitleTpl, elementName);
			// Save current element.
			btnSaveCurrentElement = string.Format (btnSaveCurrentElementTpl, elementName);
		}
		#endregion
