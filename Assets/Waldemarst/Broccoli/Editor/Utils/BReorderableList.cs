...
namespace Broccoli.Utils
{
	public class BReorderableList
	{
		...
		public delegate void ReorderCallbackDelegate(BReorderableList list);

		public delegate void SelectCallbackDelegate(BReorderableList list);

		public delegate void AddCallbackDelegate(BReorderableList list);

		public delegate void AddDropdownCallbackDelegate(Rect buttonRect, BReorderableList list);

		public delegate void RemoveCallbackDelegate(BReorderableList list);

		public delegate void ChangedCallbackDelegate(BReorderableList list);

		public delegate bool CanRemoveCallbackDelegate(BReorderableList list);

		public delegate bool CanAddCallbackDelegate(BReorderableList list);
		...
		public class Defaults
		{
			...
			public readonly GUIStyle draggingHandle = "BRL DragHandle";

			public readonly GUIStyle headerBackground = "BRL Header";

			public readonly GUIStyle footerBackground = "BRL Footer";

			public readonly GUIStyle boxBackground = "BRL Background";

			public readonly GUIStyle preButton = "BRL FooterButton";

			public GUIStyle elementBackground = new GUIStyle("BRL Element");
			...
			public void DrawFooter(Rect rect, BReorderableList list)
			{
				...
				Rect rect2 = new Rect(num + 4f, rect.y - 3f, 20f, 20f);
				Rect position = new Rect(xMax - 24f, rect.y - 3f, 20f, 20f);
			...
			public void DoAddButton(BReorderableList list)
			{
			...
			public void DoRemoveButton(BReorderableList list)
			{
				if (list.serializedProperty != null)
				{
		...
		public BReorderableList.HeaderCallbackDelegate drawHeaderCallback;

		public BReorderableList.FooterCallbackDelegate drawFooterCallback = null;

		public BReorderableList.ElementCallbackDelegate drawElementCallback;

		public BReorderableList.ElementCallbackDelegate drawElementBackgroundCallback;

		public BReorderableList.ElementHeightCallbackDelegate elementHeightCallback;

		public BReorderableList.ReorderCallbackDelegate onReorderCallback;

		public BReorderableList.SelectCallbackDelegate onSelectCallback;

		public BReorderableList.AddCallbackDelegate onAddCallback;

		public BReorderableList.AddDropdownCallbackDelegate onAddDropdownCallback;

		public BReorderableList.RemoveCallbackDelegate onRemoveCallback;

		public BReorderableList.SelectCallbackDelegate onMouseUpCallback;

		public BReorderableList.CanRemoveCallbackDelegate onCanRemoveCallback;

		public BReorderableList.CanAddCallbackDelegate onCanAddCallback;

		public BReorderableList.ChangedCallbackDelegate onChangedCallback;
		...
		private static BReorderableList.Defaults s_Defaults;

		public float elementHeight = 22f;
		...
		public static BReorderableList.Defaults defaultBehaviours
		{
			get
			{
				return BReorderableList.s_Defaults;
			}
		}
		...
		public BReorderableList(IList elements, Type elementType)
		{
			this.InitList(null, null, elements, true, true, true, true);
		}

		public BReorderableList(IList elements, Type elementType, bool draggable, bool displayHeader, bool displayAddButton, bool displayRemoveButton)
		{
			this.InitList(null, null, elements, draggable, displayHeader, displayAddButton, displayRemoveButton);
		}

		public BReorderableList(SerializedObject serializedObject, SerializedProperty elements)
		{
			this.InitList(serializedObject, elements, null, true, true, true, true);
		}

		public BReorderableList(SerializedObject serializedObject, SerializedProperty elements, bool draggable, bool displayHeader, bool displayAddButton, bool displayRemoveButton)
		{
		...
		{
			//if (BReorderableList.s_Defaults == null)
			//{
				BReorderableList.s_Defaults = new BReorderableList.Defaults();
			//}
		...
		private void DoListElements(Rect listRect)
		{
			...
			{
				BReorderableList.s_Defaults.boxBackground.Draw(listRect, false, false, false, false);
			}
			listRect.yMin += 2f;
			listRect.yMax -= 5f;
			Rect rect = listRect;
			...
			if (((this.m_Elements != null && this.m_Elements.isArray) || this.m_ElementList != null) && count > 0)
			{
				if (this.IsDragging() && Event.current.type == EventType.Repaint)
				{
					...
					for (int j = 0; j < this.m_NonDragTargetIndices.Count; j++)
					{
						if (this.m_NonDragTargetIndices[j] != -1)
						{
							...
							{
								BReorderableList.s_Defaults.DrawElementBackground(rect, j, false, false, this.m_Draggable);
							}
							else
							{
								this.drawElementBackgroundCallback(rect, j, false, false);
							}
							BReorderableList.s_Defaults.DrawElementDraggingHandle(rect, j, false, false, this.m_Draggable);
							rect2 = this.GetContentRect(rect);
							if (this.drawElementCallback == null)
							{
								if (this.m_Elements != null)
								{
									BReorderableList.s_Defaults.DrawElement(rect2, this.m_Elements.GetArrayElementAtIndex(this.m_NonDragTargetIndices[j]), null, false, false, this.m_Draggable);
								}
								else
								{
									BReorderableList.s_Defaults.DrawElement(rect2, null, this.m_ElementList[this.m_NonDragTargetIndices[j]], false, false, this.m_Draggable);
								}
							}
							else
							{
					...
					if (this.drawElementBackgroundCallback == null)
					{
						BReorderableList.s_Defaults.DrawElementBackground(rect, this.m_ActiveElement, true, true, this.m_Draggable);
					}
					else
					{
						this.drawElementBackgroundCallback(rect, this.m_ActiveElement, true, true);
					}
					BReorderableList.s_Defaults.DrawElementDraggingHandle(rect, this.m_ActiveElement, true, true, this.m_Draggable);
					rect2 = this.GetContentRect(rect);
					if (this.drawElementCallback == null)
					{
						if (this.m_Elements != null)
						{
							BReorderableList.s_Defaults.DrawElement(rect2, this.m_Elements.GetArrayElementAtIndex(this.m_ActiveElement), null, true, true, this.m_Draggable);
						}
						else
						{
							BReorderableList.s_Defaults.DrawElement(rect2, null, this.m_ElementList[this.m_ActiveElement], true, true, this.m_Draggable);
						}
					}
					else
					{
				...
				else
				{
					for (int k = 0; k < count; k++)
					{
						...
						if (this.drawElementBackgroundCallback == null)
						{
							BReorderableList.s_Defaults.DrawElementBackground(rect, k, flag2, flag3, this.m_Draggable);
						}
						else
						{
							this.drawElementBackgroundCallback(rect, k, flag2, flag3);
						}
						BReorderableList.s_Defaults.DrawElementDraggingHandle(rect, k, flag2, flag3, this.m_Draggable);
						rect2 = this.GetContentRect(rect);
						if (this.drawElementCallback == null)
						{
							if (this.m_Elements != null)
							{
								BReorderableList.s_Defaults.DrawElement(rect2, this.m_Elements.GetArrayElementAtIndex(k), null, flag2, flag3, this.m_Draggable);
							}
							else
							{
								BReorderableList.s_Defaults.DrawElement(rect2, null, this.m_ElementList[k], flag2, flag3, this.m_Draggable);
							}
						}
						else
						{
			...
			else
			{
				...
				if (this.drawElementBackgroundCallback == null)
				{
					BReorderableList.s_Defaults.DrawElementBackground(rect, -1, false, false, false);
				}
				else
				{
					this.drawElementBackgroundCallback(rect, -1, false, false);
				}
				BReorderableList.s_Defaults.DrawElementDraggingHandle(rect, -1, false, false, false);
				rect2 = rect;
				rect2.xMin += 6f;
				rect2.xMax -= 6f;
				BReorderableList.s_Defaults.DrawNoneElement(rect2, this.m_Draggable);
			}
		}
		...
		private void DoListHeader(Rect headerRect)
		{
			if (this.showDefaultBackground && Event.current.type == EventType.Repaint)
			{
				BReorderableList.s_Defaults.DrawHeaderBackground(headerRect);
			}
			headerRect.xMin += 6f;
			headerRect.xMax -= 6f;
			...
			{
				BReorderableList.s_Defaults.DrawHeader(headerRect, this.m_SerializedObject, this.m_Elements, this.m_ElementList);
			}
		}
		...
		private void DoListFooter(Rect footerRect)
		{
			...
			{
				BReorderableList.s_Defaults.DrawFooter(footerRect, this);
			}
		}
