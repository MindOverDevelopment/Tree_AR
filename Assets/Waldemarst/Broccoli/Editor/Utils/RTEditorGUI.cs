	...
	public static class RTEditorGUI 
	{
		...
		public static T ObjectField<T> (GUIContent label, T obj, bool allowSceneObjects, params GUILayoutOption[] options) where T : Object
		{
			...
			if (obj.GetType () == typeof(Texture2D)) 
			{
				...
				GUILayout.Button (obj as Texture2D, new GUILayoutOption[] { GUILayout.MaxWidth (64), GUILayout.MaxHeight (64) });
		...
		private static void SetupLineMat (Texture tex, Color col) 
		{
			if (lineMaterial == null)
				lineMaterial = new Material (Shader.Find ("Hidden/Broccoli/LineShader"));
			
			if (tex == null)
				tex = lineTexture != null? lineTexture : lineTexture = GUITextureManager.AALine;
