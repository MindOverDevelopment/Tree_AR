...
namespace Broccoli.Component
{
	...
	public class BranchBenderComponent : TreeFactoryComponent {
				...
				if (pipelineElement != null && tree != null && 
					...
					*/
					...
					if (branchBenderElement.applyNoise && !cancelNoise) {
						ApplyBranchNoise (treeFactory.treeFactoryPreferences.factoryScale);
		...
		#region Branch Noise
		private void ApplyBranchNoise (float scale) {
			for (int i = 0; i < tree.branches.Count; i++) {
				ApplyBranchNoiseRecursive (tree.branches [i], tree.branches[i].length * Random.Range (0f, 2f));
			}
		}
		private void ApplyBranchNoiseRecursive (BroccoTree.Branch branch, float lengthOffset) {
			...
			if (overrideNoiseLevels.Count > 0 && overrideNoiseLevels.ContainsKey (branch.helperStructureLevelId)) {
				...
				branch.curve.SetNoise (structureLevel.noise, structureLevel.noise,
					structureLevel.noiseScale, structureLevel.noiseScale, false, branchBenderElement.noiseResolution, branchBenderElement.noiseStrength, lengthOffset);
			} else {
				if (branch.isRoot) {
					branch.curve.SetNoise (
						...
						*/
						Mathf.Lerp (branchBenderElement.noiseScaleAtBase, branchBenderElement.noiseScaleAtRootBottom, scaleFactorAtBase),
						Mathf.Lerp (branchBenderElement.noiseScaleAtBase, branchBenderElement.noiseScaleAtRootBottom, scaleFactorAtTop),
						false,
					...
					branch.curve.SetNoise (
						...
						*/
						Mathf.Lerp (branchBenderElement.noiseScaleAtBase, branchBenderElement.noiseScaleAtTop, scaleFactorAtBase),
						Mathf.Lerp (branchBenderElement.noiseScaleAtBase, branchBenderElement.noiseScaleAtTop, scaleFactorAtTop),
						false,
						branchBenderElement.noiseResolution,
						branchBenderElement.noiseStrength,
						lengthOffset
			...
			branch.curve.ProcessAfterCurveChanged ();
			for (int i = 0; i < branch.branches.Count; i++) {
				ApplyBranchNoiseRecursive (branch.branches[i],
		...
		private void RemoveBranchNoise (BroccoTree.Branch branch = null) {
			if (branch == null) {
				...
				branch.curve.SetNoise (0f, 0f, 1f, 1f, false, 4f, 0.2f, 0f);
				branch.curve.ComputeSamples ();
				branch.curve.ProcessAfterCurveChanged ();
				for (int i = 0; i < branch.branches.Count; i++) {
