			...
			if (pipelineElement != null && treeFactory != null) {
				...
				if (trunkMeshBuilder != null) {
					...
					while (enumerator.MoveNext()) {
						...
						mesh.GetUVs (0, uvs);
						trunkJob.uvs = new NativeArray<Vector4> (uvs.ToArray (), Allocator.TempJob);

						List<Vector4> uv2s = new List<Vector4> ();
						mesh.GetUVs (1, uv2s);
						trunkJob.uv2s = new NativeArray<Vector4> (uv2s.ToArray (), Allocator.TempJob);
						...
						trunkJob.uvs.Dispose ();
						trunkJob.uv2s.Dispose ();
