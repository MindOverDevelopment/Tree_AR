	...
	public class GeometryAnalyzer {
			...
			for (int i = 0; i < _branches.Count; i++) {
				...
				for (int j = 0; j < _sprouts.Count; j++) {
					if (_sprouts [j].meshHeight > 0f) {
						point = _branches [i].GetPointAtPosition (_sprouts [j].position);
						pointForward = _branches [i].GetDirectionAtPosition (_sprouts [j].position);
						crossPoint = Vector3.Cross (pointForward.normalized, Vector3.right);
						sproutPoints.Add (point + (crossPoint * _sprouts [j].meshWidth));
						sproutPoints.Add (point + (-crossPoint * _sprouts [j].meshWidth));
						point = _branches [i].GetPointAtPosition (
							_sprouts [j].position) + 
			...
			bool isAdditive = true)
		{
			...
			for (int i = 0; i < branches.Count; i++) {
				GetSproutPositions (branches [i], lengthScale, true);
			}
		}
		/// <summary>
		...
		public void GetSproutPositions (BroccoTree.Branch branch, float lengthScale = 1f, bool isAdditive = true) {
			...
			for (int j = 0; j < _sprouts.Count; j++) {
				if (_sprouts [j].meshHeight > 0f) {
					point = branch.GetPointAtPosition (_sprouts [j].position);
					pointForward = _sprouts [j].sproutDirection.normalized;
					pointNormal = _sprouts [j].sproutNormal.normalized;

					crossPoint = Vector3.Cross (pointForward, pointNormal);

					/*
					if (_sprouts [j].groupId == 3) {
						Debug.DrawLine (point, point + crossPoint * _sprouts [j].meshWidth, Color.green, 10f);
						Debug.DrawLine (point, point + -crossPoint * _sprouts [j].meshWidth, Color.green, 10f);
						Debug.DrawLine (point, point + pointForward * _sprouts [j].meshWidth, Color.blue, 10f);
						Debug.DrawLine (point, point + pointNormal * _sprouts [j].meshDepth, Color.red, 10f); 
					}
					*/

					//sproutPoints.Add (point);
					sproutPoints.Add (point + (crossPoint * _sprouts [j].meshWidth));
					sproutPoints.Add (point + (-crossPoint * _sprouts [j].meshWidth));

					point = branch.GetPointAtPosition (_sprouts [j].position) + 
						pointForward * _sprouts [j].meshHeight;
					//sproutPoints.Add (point);
					sproutPoints.Add (point + (crossPoint * _sprouts [j].meshWidth * 1.2f));
					sproutPoints.Add (point + (-crossPoint * _sprouts [j].meshWidth * 1.2f));

					if (_sprouts [j].meshDepth > 0f) {
						point = branch.GetPointAtPosition (_sprouts [j].position) + 
							pointNormal * _sprouts [j].meshDepth * 1.3f;
						sproutPoints.Add (point + (crossPoint * _sprouts [j].meshWidth * 1.3f));
						sproutPoints.Add (point + (-crossPoint * _sprouts [j].meshWidth * 1.3f));
						sproutPoints.Add (point + (pointForward * _sprouts [j].meshWidth * 1.3f));
						sproutPoints.Add (point + (-pointForward * _sprouts [j].meshWidth * 1.3f));
					}
				}
			}
		...
		public void GetBranchOutline (
			BroccoTree.Branch branch, 
			float girthScale = 1f, 
			float resolutionAngle = 15f, 
			bool isAdditive = true)
		{
			if (!isAdditive) {
				branchPoints.Clear ();
			...
			float branchGirth = branch.maxGirth;
			points = branch.curve.GetPoints (resolutionAngle);
			for (int j = 0; j < points.Count; j++) {
				point = points [j];
				crossPoint = Vector3.Cross (point.forward.normalized, Vector3.right);
				branchPos = branch.GetPointAtPosition (point.relativePosition);
				branchPoints.Add (branchPos + (crossPoint * branchGirth * girthScale));
				branchPoints.Add (branchPos + (-crossPoint * branchGirth * girthScale));
			}
		}
		/// <summary>
		/// Gets the position of outline from a list of branches.
		/// </summary>
		/// <param name="branches">List of branches.</param>
		/// <param name="girthScale"></param>
		/// <param name="resolutionAngle"></param>
		/// <param name="isAdditive">If <c>true</c>, the points found get added to an already existing list of point.</param>
		public void GetBranchOutline (
			List<BroccoTree.Branch> branches, 
			float girthScale = 1f, 
			...
			bool isAdditive = true)
		{
			...
			for (int i = 0; i < branches.Count; i++) {
				GetBranchOutline (branches [i], girthScale, resolutionAngle, true);
			}
		}
		/// <summary>
		/// Gets the outline points of branches and sprouts from a list of branches.
		/// </summary>
		/// <param name="branches"></param>
		/// <param name="branchGirthScale"></param>
		/// <param name="branchResolutionAngle"></param>
		/// <param name="sproutLength"></param>
		/// <param name="isAdditive"></param>
		public void GetBranchAndSproutOutline (
			BroccoTree.Branch branch,
			float branchGirthScale = 1f,
			float branchResolutionAngle = 15f,
			float sproutLength = 1f,
			bool isAdditive = true)
		{
			this.GetBranchOutline (branch, branchGirthScale, branchResolutionAngle, isAdditive);
            this.GetSproutPositions (branch, sproutLength, isAdditive);
		}
		/// <summary>
		/// Gets the outline points of branches and sprouts from a list of branches.
		/// </summary>
		...
		public void GetBranchAndSproutOutline (
			...
			bool isAdditive = true)
		{
			this.GetBranchOutline (branches, branchGirthScale, branchResolutionAngle, isAdditive);
            this.GetSproutPositions (branches, sproutLength, isAdditive);
		}
		/// <summary>
