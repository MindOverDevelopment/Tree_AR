	...
	public class WeldingMeshBuilder : IBranchMeshBuilder {
			...
			BroccoTree.Branch parentBranch = null)
		{
			...
			if (rangeIndex < branchSkin.ranges.Count && branchSkin.ranges[rangeIndex].builderType == BranchMeshBuilder.BuilderType.Welding) {
				// Additional segments to the welding range.
		...
		public virtual Mesh PostprocessBranchSkinRange (Mesh mesh, int rangeIndex, BranchMeshBuilder.BranchSkin branchSkin, BroccoTree.Branch firstBranch, BranchMeshBuilder.BranchSkin parentBranchSkin = null, BroccoTree.Branch parentBranch = null) {
			...
			if (branchInfo != null && parentBranch != null) {
				...
				*/
				float weldingAngle = Vector3.SignedAngle (
					Vector3.ProjectOnPlane (firstBranch.curve.GetPointAt (0f).bitangent, directionPlane.normal),
					Vector3.ProjectOnPlane (parentBranch.GetDirectionAtPosition (firstBranch.position), directionPlane.normal),
					directionPlane.normal
				);
				...
				while (segmentIndex < branchSkin.builders.Count && branchSkin.builders [segmentIndex] == BranchMeshBuilder.BuilderType.Welding) {
					...
					inBranchWeldingLerp = Mathf.InverseLerp (branchInfo.weldingBranchSkinStartPosition, branchInfo.weldingBranchSkinEndPosition, branchSkin.directionsPosAtSkin [segmentIndex].w);
					inBranchWeldingLerp = branchWeldingCurve.Evaluate (inBranchWeldingLerp);
					// Wlding hierarchy curve.
					hierarchyWeldingLerp = inBranchWeldingLerp * 
						branchWeldingHierarchyRangeCurve.Evaluate (
							Mathf.InverseLerp (0f, (firstBranch.isRoot?rootWeldingHierarchyRange:branchWeldingHierarchyRange), branchSkin.hierarchyLevel));
					// Add segments.
					if (segmentIndex == 0) {
					...
					} else if (branchSkin.directionsPosAtSkin[segmentIndex].w >= weldingPosAtBranchSkin) {
						SetPolygonAt (
							branchSkin.id,
							firstBranch.id,
							firstBranch.helperStructureLevelId,
							//(branchSkin.segments [segmentIndex] == 0?numSegments:branchSkin.segments [segmentIndex]),
							(int)(branchSkin.idsSegs [segmentIndex].z == 0?numSegments:branchSkin.idsSegs [segmentIndex].z),
							//branchSkin.segmentTypes [segmentIndex],
							(int)branchSkin.idsSegs [segmentIndex].w,
							branchSkin.centersPos [segmentIndex],
							branchSkin.centersPos [segmentIndex].w,
							branchSkin.directionsPosAtSkin [segmentIndex],
							branchSkin.directionsPosAtSkin [segmentIndex].w,
							branchSkin.bitangentsGirths [segmentIndex],
							branchSkin.bitangentsGirths [segmentIndex].w,
							/*
							firstBranch.GetLengthAtPos (branchSkin.positions [segmentIndex], true),
							firstBranch.phase,
							firstBranch.GetPhaseLength (branchSkin.positions [segmentIndex]),
							*/
							branchSkin.phaseDirPhasePosAccumLengthStatus [segmentIndex].z,
							branchSkin.phaseDirPhasePosAccumLengthStatus [segmentIndex].x,
							branchSkin.phaseDirPhasePosAccumLengthStatus [segmentIndex].y,
							firstBranch.isRoot,
				...
				for (int i = firstSegmentIndex; i < segmentIndex; i++) {
					...
					if (i == segmentIndex - 1) {
						currSegmentSideCount = (int)branchSkin.idsSegs [segmentIndex - 1].z;
					} else {
					...
					baseIndex = topIndex;
					if (topIndex + numSegments + 1 >= uvs.Count) {
						break;
					}
				}
		...
		public BranchInfo RegisterBranchInfo (BroccoTree.Branch branch) {
			...
			if (branch.parent != null) {
				...
				float baseGirth = branch.GetGirthAtPosition (0f);
				weldingDistance = baseGirth * 2f + (baseGirth * GetWeldingDistanceFactor (branch) * 2f);
		...
		public float GetWeldingDistanceFactor (BroccoTree.Branch branch) {
			...
			weldingDistanceFactor = 1.1f; // TODO: generate the welding at the base of the branchskin.
			...
			out Vector3 weldingNormal)
		{
			...
			float baseGirth = childBranch.GetGirthAtPosition (0f);
			float colliderRange = baseGirth + branchInfo.weldingLowerSpread * baseGirth;
			colliderRange = colliderRange / parentBranchSkin.length;
			MeshCollider meshCollider = GenerateBranchMeshCollider (positionAtParentBranchSkin, colliderRange, parentBranch, parentBranchSkin);
			...
			//Vector3 rayDirection = Quaternion.AngleAxis (childBranch.rollAngle * Mathf.Rad2Deg, parentDirection) * parentNormal; // FORMER
			...
			rayDirection = (childBranch.GetDirectionAtPosition (0f) + rayDirection) /2f;

			//float girthAtChild = childBranchSkin.GetGirthAtLength (girthAtParent, childBranch);
			float girthAtChildBase = childBranch.GetGirthAtPosition (0f);
			float girthAtChild = childBranchSkin.GetGirthAtLength (girthAtChildBase, childBranch);
			...
			if (polygonSides >= 3) {
				...
				//Vector3 center = parentBranchSkin.GetPointAtPosition (positionAtParentBranchSkin, parentBranch) * globalScale; // FORMER
				Vector3 direction = rayDirection;
				//Vector3 normal = childBranch.GetNormalAtPosition (0f);
				Vector3 normal = childBranch.curve.GetPointAt (0f).bitangent;
				...
				if ((childBranch.isRoot && useRootWeldingMeshCap) || (!childBranch.isRoot && useBranchWeldingMeshCap)) {
					for (int i = 0; i <= polygonSides; i++) {
						...
						uv2s.Add (new Vector4 (vertex.x, vertex.y, vertex.z, 0f));
						...
						uv7s.Add (center);
						*/
						float accumLength = childBranch.GetLengthAtPos (0f, true);
						float childGirth = childBranch.GetGirthAtPosition (0f);
						uvs.Add (new Vector4 (
							radialPosition, 
							accumLength, 
							radialPosition, 
							childGirth));
						//uv2s.Add (new Vector4 (vertex.x, vertex.y, vertex.z, 0f));
						uv2s.Add (new Vector4 (
							accumLength, 
							MeshUtils.Vector3ToFloat (childBranch.phaseDir), 
							childBranchSkin.phaseDirPhasePosAccumLengthStatus [0].y, 
							childBranch.isRoot?1:0));
						uv5s.Add (new Vector4 (
							childBranch.id, 
							childBranchSkin.id,
							childBranch.helperStructureLevelId));
						uv6s.Add (new Vector4 (center.x, center.y, center.z, childBranch.isTuned?1:0));
						uv7s.Add (new Vector4 (direction.x, direction.y, direction.z));
						uv8s.Add (new Vector4 (radialPosition, accumLength, childGirth));
						/*
						uv5s.Add (new Vector4 (radialPosition, childBranchSkin.lengthOffset, girthAtChild, 0f));
						uv6s.Add (new Vector4 (childBranch.id, childBranchSkin.id, childBranch.helperStructureLevelId, 0f));
						uv7s.Add (center);
				...
				for (int i = 0; i <= polygonSides; i++) {
					if (i != polygonSides) {
						...
						vertex = (rotation * vertex) + center - rayDirection * girthAtParent * 0.3f;
						vertexNormal = (vertex - center).normalized;
						float rayScale = 2f;
						if (childBranch.id == debugBranchId) {
							Debug.DrawLine (vertex, vertex + rayDirection.normalized * girthAtParent * rayScale, Color.yellow, 3f);
						}
						if (meshCollider.Raycast (new Ray (vertex, rayDirection), out hitInfo, girthAtParent * rayScale)) {
							//Debug.DrawLine (vertex, vertex + (rayDirection.normalized * girthAtParent * rayScale), Color.blue, 2.5f);
							vertex = hitInfo.point;
							vertexNormal = hitInfo.normal;
							if (childBranch.id == debugBranchId) {
								Debug.DrawLine (
									hitInfo.point, 
									vertex + rayDirection.normalized * girthAtParent * rayScale, 
									Color.Lerp (Color.yellow, Color.red, 0.5f), 
									3f);
							}
							//Debug.Log ("Hit found option A.");
						} else if (meshCollider.Raycast (new Ray (center, vertex - center), out hitInfo, girthAtParent * rayScale)) {
							//Debug.DrawLine (center, center + ((vertex - center)).normalized * girthAtParent * rayScale, Color.blue * 1.4f, 2.5f);
							vertex = hitInfo.point;
							vertexNormal = hitInfo.normal;
							//Debug.Log ("Hit found option B.");
						} else {
							//Debug.DrawLine (center, center + ((vertex - center)).normalized * girthAtParent * rayScale, Color.red, 2.5f);
					...
					*/
					...
					uv5s.Add (new Vector4 (
						...
						childBranchSkin.id,
						childBranch.helperStructureLevelId));
					uv6s.Add (new Vector4 (center.x, center.y, center.z));
					uv7s.Add (new Vector4 (direction.x, direction.y, direction.z));
					uv8s.Add (new Vector4 (radialPosition, accumLength, childGirth));
				}
			}
			return girthAtParent; // FORMER
			//return girthAtChildBase;
		}
		void SetPolygonAt (
			int branchSkinId,
			int branchId,
			...
			BranchInfo branchInfo)
		{
			...
			if (polygonSides >= 3) {
				...
				for (int i = 0; i <= polygonSides; i++) {
					...
					uv6s.Add (new Vector4 (center.x, center.y, center.z));
					uv7s.Add (new Vector4 (direction.x, direction.y, direction.z));
					uv8s.Add (new Vector4 (radialPosition, lengthOffset, girth));
					indexPos++;
				}
			}
		}
		...
		public MeshCollider meshColl = null;
		/// <summary>
		...
		public MeshCollider GenerateBranchMeshCollider (float positionAtSkin, float range, BroccoTree.Branch firstBranch, BranchMeshBuilder.BranchSkin branchSkin) {
			...
			//MeshCollider meshCollider = InnerGetMeshCollider ();
			...
			int lastI = 0;
			for (int i = 0; i < branchSkin.totalSegments; i++) {
				posAtSkin = branchSkin.directionsPosAtSkin [i].w;
				if (posAtSkin > bottomRange && posAtSkin < topRange) {
					if (segments.Count == 0) segments.Add ((int)(branchSkin.idsSegs [i].z == 0?defaultNumSeg:branchSkin.idsSegs [i].z));
					segments.Add ((int)(branchSkin.idsSegs [i].z == 0?defaultNumSeg:branchSkin.idsSegs [i].z));
					segmentPositions.Add (posAtSkin);
					lastI = i;
				} else if (posAtSkin > topRange) {
					if (segments.Count == 0) {
						segments.Add ((int)(branchSkin.idsSegs [i].z == 0?defaultNumSeg:branchSkin.idsSegs [i].z));
						segments.Add ((int)(branchSkin.idsSegs [i].z == 0?defaultNumSeg:branchSkin.idsSegs [i].z));
						segmentPositions.Add (posAtSkin);
			...
			segments.Add ((int)(branchSkin.idsSegs [lastI].z==0?defaultNumSeg:branchSkin.idsSegs [lastI].z));
