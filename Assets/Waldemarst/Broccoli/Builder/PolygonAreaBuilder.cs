...
using Fragment = Broccoli.Factory.SnapshotProcessor.Fragment;
...
namespace Broccoli.Builder
{
    public class PolygonAreaBuilder {
        ...
        public FragmentBias GetFragmentationBias (
            int maxLevel, 
            int lod, 
            out int fragLevels,
        ...
        public void GenerateHullPoints (PolygonArea polygonArea, Fragment fragment) {
        ...
        private void CreatePolygonNonConvexHull (PolygonArea polygonArea, Fragment fragment, ProcessControl pc = null) {
        ...
        private void CreatePolygonNonConvexHull_Old (PolygonArea polygonArea, Fragment fragment) {
            // Get the geometry analizer and set the snapshot points.
            GeometryAnalyzer ga = GeometryAnalyzer.Current ();
            List<Vector3> snapshotPoints = new List<Vector3> ();
            List<Vector3> snapshotHull = new List<Vector3> ();
            ...
            for (int i = 0; i < _filteredBranches.Count; i++) {
                ...
                if (branchLevel <= 1) {
                    ...
                    if (branchLevel == 0) {
                        ...
                        List<BroccoTree.Branch> subBranches = ga.GetOutlinePoints (_filteredBranches [i], true);
                        //ga.GetSproutPositions (subBranches, 1.3f, false);
                        ga.GetSproutPositions (subBranches, 1.3f, true);
                        snapshotPoints.Add (_filteredBranches [i].GetPointAtPosition (0f));
        ...
        private void CreatePolygonConvexHull (PolygonArea polygonArea, Fragment fragment, ProcessControl pc = null) {
            if (pc == null) pc = ProcessControl.GetDefault ();
            ...
            ga.GetBranchAndSproutOutline (
                ...
                pc.topoSproutLengthScale);
            /*            
            ga.GetBranchOutline (_filteredBranches, 2f, 45f);
            snapshotPoints.AddRange (ga.branchPoints);
            // Get Sprout points.
        ...
        public void ProcessPolygonDetailPoints (PolygonArea polygonArea, Fragment fragment) {
            if (polygonArea.resolution > 0) {
                ...
                if (polygonArea.resolution == PolygonArea.MAX_RESOLUTION - 1) {
                    ...
                    polygonArea.points.Add (pointToAdd);
        ...
        public void ProcessPolygonAreaMesh (PolygonArea polygonArea, Bounds refBounds) {
            // Triangulation
            GeometryAnalyzer ga = GeometryAnalyzer.Current ();
