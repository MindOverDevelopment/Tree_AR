...
namespace Broccoli.Builder
{
	public class BillboardBuilder {
		...
		public GameObject GenerateBillboardAsset (GameObject target, bool isST8) {
			...
			if (CreateBillboardTexture (target, billboardTexturePath, billboardTexture, unlitMaterials, meshRenderer, Color.clear)) {
				CreateBillboardTexture (target, billboardNormalTexturePath, billboardNormalTexture, normalMaterials, meshRenderer, new Color (0.5f, 0.5f, 1f, 1f));
				if (!isST8) {
		...
		private bool CreateBillboardTexture (GameObject target, string texturePath, Texture2D targetTexture, Material[] materials, MeshRenderer meshRenderer, Color bgColor) {
			#if UNITY_EDITOR
		...
		private void DebugDrawCamera (Transform cameraTransform, Vector3 target, Color color) {
			Debug.DrawLine (cameraTransform.position, target, color, 10f);
		}
